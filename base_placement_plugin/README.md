...
1. Only the rviz panel (with all the required displays) and the code (useful with big simulation already running):
  ...
  - *Filtering parameters*: to choose and setup the options for the filtering of the Union Map (see related section of the Readme)
    ...
    - **OCTOMAP_SRV_filtering** : bool to have the filtering using a OCTOMAP from the octomap_server (3-dimentional) (alterinative method to the planning scene one) - default = *false*
      - **OCTOMAP_srv_topic** : name of the topic to retreive the OGM (make sure that it is a topic of type *octomap_msgs/Octomap*) - default=*"/octomap_full"*
...
- Collision filtering: poses of both the arm base and the robot base must not be in collision with the objects of the scene. This filtering can be done using different resources (also depending on the robot used): 
  ...
  To retreive the octomap of the environment there are two main options:
    ...
    - Planning Scene OCTOMAP **NOT IMPLEMENTED YET**: thanks to the [PERCEPTION PIPELINE](https://moveit.github.io/moveit_tutorials/doc/perception_pipeline/perception_pipeline_tutorial.html) of MoveIt (check your robot moveit_config pkg to make sure it is set if you want to use it) the Planning Scene can contain information on the environment in form of an octomap (created using either PointCloud or Depth image sensors).  
    The octomap of the planning scene is used for collision checks during motion planning, to recognize objects and create collision objects (for grasping tasks) and more. 
    ...
    **!!!** For the method VerticalRobotModel the OCTOMAP as is is not very useful (since we only check ground height points - don't have complete information on what's above), so when requesting a filtering with octomaps but at the same time wanting to use this method, the filtering has to be done some other way:
    - the octomap_server provides the projection of the octomap on the ground through the topic **/projeted_map** with msgs of type *nav_msgs/OccupancyGrid*: we can then use this map "recycling" the code from the costmap filtering. **BUT**  this map contains binary occupancy info and does NOT have an inflation layer to account for navigation feasibility and considers the entire octomap so MAKE SURE to use the ground filter or the projected map will show everything occupied. Therefore is not advised to be used (better to use the costmap if available in this case)
    - the planning scene natively does not provide a similar topic so the filtering will be perfomed by checking the x,y poses in "columns" (given x0,y0 to check I iterate through x0,y0,z points of the octomap with z from just above the ground to the max_z of the octree bounding box)
