...
- Collision filtering: poses of both the arm base and the robot base must not be in collision with the objects of the scene. This filtering can be done using different resources (also depending on the robot used): 
  ...
  To retreive the octomap of the environment there are two main options:
    ...
    - Planning Scene OCTOMAP **NOT IMPLEMENTED YET**: in some cases the [PERCEPTION PIPELINE](https://moveit.github.io/moveit_tutorials/doc/perception_pipeline/perception_pipeline_tutorial.html) of MoveIt is implemented (check your robot moveit_config pkg). This allows to have the octomap information included in the Planning Scene, to use it for motion planning with collision checks. **to be checked**  this octomap has some kind of inflation
  - **!!!** For the method VerticalRobotModel the OCTOMAP as is is not very useful (since we only check ground height points - don't have complete information on what's above), the octomap_server however provides the projection of the octomap on the ground through the topic **/projeted_map** with msgs of type *nav_msgs/OccupancyGrid*; we can then use this map for the filtering in this case. **BUT**  this map contains binary occupancy info and does NOT have an inflation layer to account for navigation feasibility. Therefore is not advised to be used (better to use the costmap if available in this case)
