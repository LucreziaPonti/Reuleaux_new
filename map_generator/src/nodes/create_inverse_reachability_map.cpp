...
{
ros::init(argc, argv, "create_inverse_reachability_map");
  ros::NodeHandle n;
  ...
  if (argc < 2)
  {
    ROS_ERROR_STREAM("USAGE: create_inverse_reachability_map <hdf5_file_path> <output_file_name> \n"
                     "output_file_name IS OPTIONAL - if not provided, the output file will be created with the name <input_file>_INVERSE.h5");
    return 0;
  }
  ...
  else if (argc == 2)
  {
    ...
    {
      // create the file variable by taking the input_file and adding "_INVERSE" to the name
      std::string str(input_FILE);
      std::string::size_type pos = str.find_last_of(".");
      if (pos == std::string::npos)
      {
        ROS_ERROR("File name does not have an extension");
        return false;
      }
      std::string base_name = str.substr(0, pos);
...
/*/////////////////////////////////////////////////////////////
  ...
  while (ros::ok())
  {
    ...
    sd.createPosesOnSphere(origin, resolution, pose);  // calculating number of points on a sphere by discretization
    ...
    utility::MultiMapPtr base_trns_col;
...
//      octree.voxelSearch(search_point, point_idx_vec);
   ...
   for (utility::MultiMapPtr::iterator it = base_trns_col.begin(); it!=base_trns_col.end(); ++it)
   {
