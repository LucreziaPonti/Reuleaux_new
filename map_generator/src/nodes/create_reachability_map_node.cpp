#include <ros/ros.h>
#include <map_generator/map_generation.h>



int main(int argc, char **argv)
{
  ros::init(argc, argv, "create_reachability_map_node");
  ros::NodeHandle nh_("~");



  std::string group_name_, path_, filename_, pkg_name_;;
  bool check_collision_;
  bool do_centering_;
  double resolution_, radius_;
  nh_.getParam("group_name", group_name_);
  nh_.getParam("resolution", resolution_);
  nh_.getParam("radius", radius_);
  nh_.getParam("check_collision", check_collision_);
  nh_.getParam("do_centering", do_centering_);
  nh_.getParam("path", path_);
  nh_.getParam("filename", filename_);
  nh_.getParam("pkg_name", pkg_name_);

  map_generation::mapGeneration mg(nh_, group_name_, path_, filename_,pkg_name_,
                             resolution_, radius_, check_collision_,do_centering_);

  ros::Duration(5).sleep();
  mg.generate();

  ros::spin();
  return 0;
}

