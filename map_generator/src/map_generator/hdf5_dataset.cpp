...
bool Hdf5Dataset::saveMap(const utility::VecVecDouble &pose_reach, const utility::VecVecDouble &spheres, const utility::VecDouble &ri, const double resolution)
{
...
bool Hdf5Dataset::saveWorkspaceToMap(const map_generator::WorkSpace &ws)
{
  ...
  for(int i=0;i<ws.WsSpheres.size();++i)
  {
    ...
    utility::pointToVector(ws.WsSpheres[i].point, sphere_vec);
    ...
    for(int k=0;k<ws.WsSpheres[i].poses.size();++k)
    {
      ...
      utility::poseToVector(ws.WsSpheres[i].poses[k], pose_vec);
      for(int l=0;l<7;++l)
      {
  ...
  ROS_DEBUG("===================================");
  ROS_DEBUG("RETRIEVED WORKSPACE RESOLUTION: %f", ws.resolution);
  ROS_DEBUG("===================================");
...
bool Hdf5Dataset::save(const map_generator::WorkSpace &ws)
{
  ws_ = ws;
...
bool Hdf5Dataset::saveReachData( utility::MultiMapPtr& poses,  utility::MapVecDoublePtr& spheres, float resolution){
  
  //Creating Multimap to Straight vector<vector<double> > //Can take this function to a new class
  std::vector< std::vector< double > > pose_reach;
  
  for (utility::MultiMapPtr::iterator it = poses.begin(); it != poses.end(); ++it)
  {
  ...
  std::vector<std::vector<double> > spheres_coord; 
  std::vector<double> ri;
  for (utility::MapVecDoublePtr::iterator it =  spheres.begin(); it !=spheres.end(); ++it){
    std::vector<double> sphere_vec = *it->first;
    spheres_coord.push_back(sphere_vec);
    ri.push_back(it->second);
  }
  saveMap(pose_reach, spheres_coord, ri, resolution);
  return true;
}
...
bool Hdf5Dataset::h5ToMultiMapPosesAndSpheres(utility::MultiMapPtr& pose_col, utility::MapVecDoublePtr& sphere_col)
{
  ...
  utility::MultiMap sphere_and_poses;
  utility::MapVecDouble sps;
  h5ToMultiMapPoses(sphere_and_poses, sps);

  utility::MapVecDouble sp_col;
  h5ToMultiMapSpheres(sp_col);
  
 for(utility::MapVecDouble::iterator it=sps.begin(); it!=sps.end();++it)
  {
    ...
    (*sp)[1] = (it->first)[1];
    (*sp)[2] = (it->first)[2];
    for(utility::MultiMap::iterator it1= sphere_and_poses.lower_bound(it->first); it1 != sphere_and_poses.upper_bound(it->first); ++it1)
    {
    ...
    for (utility::MapVecDouble::iterator it2 = sp_col.lower_bound(it->first); it2 !=sp_col.upper_bound(it->first); ++it2)
    {
...
bool Hdf5Dataset::h5ToMultiMapPoses(utility::MultiMap& pose_col)
{
  utility::MapVecDouble sphere_col;
  h5ToMultiMapPoses(pose_col, sphere_col);
  return true;
}

bool Hdf5Dataset::h5ToMultiMapPoses(utility::MultiMap& pose_col, utility::MapVecDouble& sphere_col)
{
...
bool Hdf5Dataset::h5ToMultiMapSpheres(utility::MapVecDouble& sphere_col)
{
...
bool Hdf5Dataset::loadMapsFromDataset(utility::MultiMapPtr& poses, utility::MapVecDoublePtr& spheres, float &resolution)
{
...
bool Hdf5Dataset::loadMapsFromDataset(utility::MultiMapPtr& poses, utility::MapVecDoublePtr& spheres)
{
  h5ToMultiMapPosesAndSpheres(poses, spheres);
  close();
  return 0;
...
bool Hdf5Dataset::loadMapsFromDataset(utility::MultiMap& poses, utility::MapVecDouble& spheres)
{
...
bool Hdf5Dataset::loadMapsFromDataset(utility::MultiMap& poses, utility::MapVecDouble& spheres, float &resolution)
{
  h5ToMultiMapPoses(poses);
  h5ToMultiMapSpheres(spheres);
  getResolution(resolution);
